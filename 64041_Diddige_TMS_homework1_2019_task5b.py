...
def plot_trajectories(times,vels):
    ...
    plt.xlabel(r'velocity of Dislocation [m/s]')
    plt.ylabel('time[ns]')
    plt.title('velocity of dislocations $(t_{0}:circles,t_{end}:diamonds)$')
    positions_scaled=positions/1e-6         #positions scaled by e-6
    times_scaled=times/1e-9                 #times scaled by e-9
    plt.plot(vels[0:],times_scaled[0:],'b--',vels[0:1],times_scaled[0:1],'bo',vels[-1:],times_scaled[-1:],'yD')
    plt.savefig('Task_5b_img.png')
